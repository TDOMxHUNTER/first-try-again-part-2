...
interface ProfileCardProps {
  ...
  onContactClick?: () => void;
  onProfileUpdate?: (data: { name?: string; title?: string; handle?: string; avatarUrl?: string; status?: string }) => void;
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  }, [showSettings, name, title, handle, status]);
                      ...
                      img.onload = () => {
                        ...
                        if (imageData) {
                          ...
                          for (let i = 0; i < data.length; i += 4) {
                            ...
                            const isGrayish = saturation < 30;
                            const isLight = brightness > 220;

                            // Remove white/light backgrounds and very uniform colors
                            if (isLight || (isGrayish && brightness > 180)) {
                              data[i + 3] = 0; // Make transparent
                            }
                            // Also check for common background colors (light gray, beige, etc.)
                            else if (
                              (r > 240 && g > 240 && b > 230) || // Off-white
                              (r > 230 && g > 230 && b > 220 && Math.abs(r - g) < 20 && Math.abs(g - b) < 20) // Light gray/beige
                            ) {
                              data[i + 3] = 0; // Make transparent
                            }
                          }
                        ...
                        if (onProfileUpdate) {
                          onProfileUpdate({ avatarUrl: result });
                        }
                      };
